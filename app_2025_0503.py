...
def remove_overlap(prev: str, curr: str) -> str:
    """
    找到 prev 結尾與 curr 開頭之間的最大重疊，刪掉 curr 的這部分，
    回傳剩餘 curr 的尾部；若無重疊，回傳 curr 原文。
    """
...
def search_ajax():
    ...
    for key, value in results.items():
        ...
        if "sentences" in value:
            for _, snippet_list in value["sentences"].items():
        ...
        # 3) 針對出現>=2次且非第一段的段落，做前後綴重疊截斷去重
        processed = []
        prev_para = None
        for para in deduped:
            occs = [m.start() for m in re.finditer(re.escape(keyword), para)]
            # 若本段出現少於 2 次，或是第一段，直接保留
            if len(occs) < 2 or prev_para is None:
                processed.append(para)
                prev_para = para
                continue

            # 否則用 remove_overlap 刪除與 prev_para 的最大重疊
            trimmed = remove_overlap(prev_para, para)
            if trimmed.strip():
                processed.append(trimmed)
            # 不論是否保留，都將本段設為下一輪的 prev_para
            prev_para = para
    ...
    return jsonify({ "data": transformed, "_stat_": results["_stat_"] })
